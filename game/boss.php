...
if ($boss->bosshp <=0 ){//这里是对于BOSS没生命了进行刷新的人进行提示
		$sql = "update boss set bossmaxhp = bossmaxhp+100  WHERE bossid='$bossid'";//<!--给boss加血-加属性-->
		$dblj->exec($sql);
		$sql = "update boss set bossgj = bossgj+5  WHERE bossid='$bossid'";//<!--给boss加血-加属性-->
		$dblj->exec($sql);
	$html = <<<HTML
	<IMG width='280' height='140' src='./images/boss.png'src="./images/rw.png" style="border-radius: 8px;">
        BOSS已经逃跑，慢慢的BOSS学会了成长，不再哭泣，战斗力越来越吊炸天！<br/>
        <br/>
...
elseif ($cmd == 'pvbgj' && $bossid != 0){
   ...
   // }
    ...
    $phurt = 0 ;
	$gwsj = mt_rand(1,100);//控制怪物暴击概率
    if ($boss->bossbj >= $gwsj){
        $boss->bossgj = round($boss->bossgj * 2.25);
        $gwbj = '重击';
    }

    $phurt = round($boss->bossgj - ($player->ufy * 0.75),0);//boss攻击计算
    ...
    $pvbxx = ceil($gphurt * ($player->uxx/200) );//控制吸血强度
    ...
    if ($boss->bosshp <=0 ){//怪物死亡,奖励计算
		...
		$sjyxb = mt_rand(1,5);
        $yxb = round($boss->bosslv*$sjyxb*30) + $sjyxb*110+$sjyxb1;//游戏币计算
        if ($hurt || $lvc >=35){
            $yxb = 0;//大于35五级游戏币为零
        }
...
$tz = <<<HTML
					...
					[{$player->uname}]{$jisha}[{$boss->bossname}]获得<a href='?cmd=zbinfo_sys&zbid=$zbys->zbid&sid=$sid' style="background-color: #f6f7f7;padding: 0px 0px;"><font style="color:{$zbys->zbys}">[{$zbname}]</font></a>令人羡慕.
HTML;
                    $stmt->execute(array('【号外】',$tz,'0'));//系统公告
					}
            }
        }
		//道具计算
        ...
        if ($boss->djjv >= $sjjv && $boss->bossdj != ''){
            ...
            if ($cxdljg){
                ...
                $huode .= "获得:<font class='djys'>$djname x$djsum</font><br>";//色彩要素
                for ($i=0;$i<count($taskarr);$i++){
                    $rwyq = $taskarr[$i]['rwyq'];
                    $rwid = $taskarr[$i]['rwid'];
                    $rwzl = $taskarr[$i]['rwzl'];
...
HTML;
        ...
        case 0:
			...
			$sql = "insert into ggliaotian(name,msg,uid) values(?,?,?)";
            $stmt = $dblj->prepare($sql);
            $stmt->execute(array('【号外】',"[{$boss->bossname}]正在戏弄[{$player->uname}]...",'0'));//系统公告
			}
            $html = <<<HTML
            战斗结果:<br/>
...
if ($cmd == 'taopao'){
    ...
    if ($sji>= 80 ){
		...
		header("refresh:1;url=?cmd=$gonowmid");//延迟跳转1秒
    }else{
		$tx = '逃跑失败<br>';
	}
}
...
气血:(<div class="hpys" style="display: inline">$boss->bosshp</div>/<div class="hpys" style="display: inline">$boss->bossmaxhp</div>)$pvbsb$pvbbj$ghurt<br/>
攻击:($boss->bossgj)<br/>
防御:($boss->bossfy)<br/>
当前攻击人数:<br/>
===================<br/>
$player->uname [lv:$player->ulv]<br/>
气血:(<div class="hpys" style="display: inline">$player->uhp</div>/<div class="hpys" style="display: inline">$player->umaxhp</div>)$gwbj$phurt$pvbxx<br/>
攻击:($player->ugj)<br/>
防御:($player->ufy)<br/>
$tishihtml
<br/>
