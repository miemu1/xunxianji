 ...
 * Created by PhpStorm.
 * User: Administrator
 * Date: 2021 /12/19 准备修改宠物出站 和属性成长
 * Time: 22:22
 */
//if (!isset($uid)){
//
...
if ($pvper->uhp<=0){
    ...
    $tishihtml = '<FONT color="#ff7888" size="3.5&sid">该玩家已经受到暴力一击，当前正垂死挣扎，还请大侠高抬贵手。</FONT><br/><br/>'.
    ...
    switch ($canshu){
        case 'gj':
            ...
            if ($pvper->uhp<=0){
                ...
                $msg = $dieinfo[$randdie];
                $sql = "insert into ggliaotian(name,msg,uid) values('【传闻】','$msg','0')";
                $cxjg = $dblj->exec($sql);
                $html = '
                    战斗结果:<br/>
                    你打死了'.$pvper->uname.'<br/>
