...
if ($player->uid == $zhuangbei->uid && !in_array($zhuangbei->zbnowid,$arr)){
    ...
    $setzbwz = "<a href='?cmd=$setzbwz'>穿戴装备</a>";
    ...
    <a href='?cmd=$setzbwz2'>装备在【防具】位置</a><br/>
    <a href='?cmd=$setzbwz3'>装备在【首饰】位置</a>
    <a href='?cmd=$setzbwz4'>装备在【书籍】位置</a><br/>
    <a href='?cmd=$setzbwz5'>装备在【坐骑】位置</a>
    <a href='?cmd=$setzbwz6'>装备在【令牌】位置</a><br/>
    <a href='?cmd=$setzbwz7'>装备在【暗器】位置</a>";
    }

    $setzbwz .=<<<HTML
    <a href="?cmd=$delezb" style="float:right;">分解装备</a>
    <br/>
    <form action="$self">
...
$qhcgl = round((30-$zhuangbei->qianghua)/30,2) * 100;//装备强化成功率计算
$qhcgl .='%';
$tools = array("不限定","武器","防具","首饰","书籍","坐骑","令牌","暗器");
$tool = $tools[$zhuangbei->tool];


$html = <<<HTML
装备名称:<font color='$zhuangbei->zbys'>【 $zhuangbei->zbname 】</font>$qianghua<br/>
...
<!--
增加气血:$zhuangbei->zbhp$uphp<br/>
装备吸血:$zhuangbei->zbxx$upxx<br/>开启吸血强化
-->

=======<br>
种类:<font style="color: #f90909;">$tool</font><br>
=======<br>
